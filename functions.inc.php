...
function brand_product($con,$limit='',$product_id='',$brand_id='',$sort_order=''){
    $sql="select product.*,brands.brands from product,brands where product.status=1 ";
    if($brand_id!=''){
		$sql.=" and product.brands_id=$brand_id ";
	}
	
	if($product_id!=''){
		$sql.=" and product.id=$product_id ";
	}
	$sql.=" and product.brands_id=brands.id ";
